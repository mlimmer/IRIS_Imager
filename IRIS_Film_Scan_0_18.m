...
[Parameters,ParameterText]=xlsread(ParamFile,'Parameters','B2:B25');

DPI=Parameters(1); %image resolution in pixels/inch
x_Crop=Parameters(2); %number of pixels to crop off x direction
y_Crop=Parameters(3); %number of pixels to crop off y direction
Scale_Factor=Parameters(4); %Ratio to scale the image by averaging pixels i.e., 0.5 = 1/2 size
Start_Upside_down_flag=Parameters(5); %Flag for upsidedown scanned in images 1=upsidedown
End_Upside_down_flag=Parameters(6); %Flag for upsidedown scanned in images 1=upsidedown
White_L=Parameters(7); %Lightness value for white film (LAB)
Fe_b_Threshold=Parameters(8); %Values of b* change above this are considered Fe (rather than white) for counting Fe on Mn films
CircleRad_min=Parameters(9)*DPI*Scale_Factor; %Min cirlce radius in pixels;
CircleRad_max=Parameters(10)*DPI*Scale_Factor; %Max cirlce radius in pixels;
Crop_Buffer=Parameters(11); %Factor for cropping circles to increase image area by a factor of 2
Circle_NaN_buffer=Parameters(12); %Factor for increasing NaN masking around circles (multiplied by circle radius)
n_circles_start=Parameters(13); %number of circles in starting image
n_circles_end=Parameters(14); %number of circles in ending image
n_clusters_start=Parameters(15); %number of clusters to find in starting image
n_clusters_end=Parameters(16); %number of clusters to find in ending image
OutputRemoval=Parameters(17); %flag: 1 to output removal with depth, 0 to supress output
OutputRemovalDepth=Parameters(18); %Distance over which to average removal with depth (inches). Use 0 without averaging
IRIS_Type=ParameterText{1}; %Type of IRIS film analyzed -- Fe, Mn or S
OutFile=ParameterText{2}; %Output data file name
OutSheet=ParameterText{3}; %Output datasheet
Start_circle_color=ParameterText{4}; %Color of circle relative to sheet for starting image
End_circle_color=ParameterText{5}; %Color of circle relative to sheet for ending image
Image_Path=ParameterText{6}; %Default file path for images
    ...
    switch IRIS_Type
        case 'S'
            ...
            if miny>size(I_end,1)
                fprintf('WARNING!\nFilm height is bigger than the scanned images!\n');
            end
            ...
            for i=1:n_circles_end
                ...
                I_end_circle=I_end(max([1 End_Circle_ID(2)-CircleRad_max*Crop_Buffer]):min(size(I_end,1),End_Circle_ID(2)+CircleRad_max*Crop_Buffer),End_Circle_ID(1)-CircleRad_max*Crop_Buffer:End_Circle_ID(1)+CircleRad_max*Crop_Buffer,:); %Crop a small sqaure around the hole
            ...
            save(erase(End_file,'.'),'I_end','end_mask','World_end');
        ...
        case {'Fe','Mn'}     
            ...
            if miny>size(I_start,1) || miny>size(I_end,1)
                fprintf('WARNING!\nFilm height is bigger than one of the scanned images!\n');
            end
            ...
            for i=1:n_circles_start
                ...
                I_start_circle=I_start(max([1 Start_Circle_ID(2)-CircleRad_max*Crop_Buffer]):min(size(I_start,1),Start_Circle_ID(2)+CircleRad_max*Crop_Buffer),Start_Circle_ID(1)-CircleRad_max*Crop_Buffer:Start_Circle_ID(1)+CircleRad_max*Crop_Buffer,:); %Crop a small sqaure around the hole
                [Start_Circle_center, Start_Circle_radii]=CIRCLE_HUNTER(I_start_circle,[CircleRad_min, CircleRad_max],Start_circle_color);
            ...
            for i=1:n_circles_end
                End_Circle_ID=CIRCLE_ID(h_circles);
                I_end_circle=I_end(max([1 End_Circle_ID(2)-CircleRad_max*Crop_Buffer]):min(size(I_end,1),End_Circle_ID(2)+CircleRad_max*Crop_Buffer),End_Circle_ID(1)-CircleRad_max*Crop_Buffer:End_Circle_ID(1)+CircleRad_max*Crop_Buffer,:); %Crop a small sqaure around the hole
                [End_Circle_center, End_Circle_radii]=CIRCLE_HUNTER(I_end_circle,[CircleRad_min, CircleRad_max],End_circle_color);
                End_Circle_center=End_Circle_center+[End_Circle_ID(1)-CircleRad_max*Crop_Buffer,End_Circle_ID(2)-CircleRad_max*Crop_Buffer]; %Put the coordinates back into the global coordinate system
                figure(h_circles); %bring figure to front
                End_Circle_center=round(End_Circle_center);
                End_Circle_radii=round(End_Circle_radii);
            ...
            %% Save Images using the starting file name
            save(erase(End_file,'.'),'I_start','I_end','start_mask','end_mask','World_start','World_end');
        otherwise
...
%% Output Data
if isfile(OutFile)
    existing_output=xlsread(OutFile,OutSheet);
    ...
    xlswrite(OutFile,Headers,OutSheet);
end
...
switch IRIS_Type %Adjust output data based upon film type
    case 'Mn'
        ...
        Outdata=[Start_file End_file num2cell([x_width(length(x_width)), y_length(length(y_length)),...
            ...
            num2cell([f.a,f.b,f.c,f.d,f.e,f_noFe.a,f_noFe.b,f_noFe.c,f_noFe.d,f_noFe.e])];
    ...
    case 'S'
        ...
        % in a* and b*, starting cluster area, starting cluster colors, ending
        % cluster area, ending cluster colors, Removal fit values
        Outdata=[End_file num2cell([x_width(length(x_width)), y_length(length(y_length)),...
            ...
            num2cell([f_S.a,f_S.b,f_S.c,f_S.d,f_S.e])];     
end

%Write file
status=xlswrite(OutFile,Outdata,OutSheet,['A' num2str(existing_rows+1)]);
...
if OutputRemoval==1
    switch IRIS_Type
        case 'Mn'
            RemovalHeaders={'Depth (in)','Average Mn Removal','Average Removal Assuming Fe Pixels as 100% Removal (For Mn Films)'};
   ...
   [~,output_sheets]=xlsfinfo(OutFile);
   if sum(strcmp(output_sheets,End_file))
       existing_output_removal=xlsread(OutFile,End_file);     
   ...
   status_rem=xlswrite(OutFile,[RemovalHeaders;num2cell(Removal_Avg)],End_file,['A' num2str(existing_rows_removal+1)]);
...
ParamFile='AdjustableParameters.xlsx'; %Parameter file name
[Munsell_LAB,Munsell_Names]=xlsread(ParamFile,'Munsell');
Munsell_Names=Munsell_Names(2:size(Munsell_Names,1),1:1);

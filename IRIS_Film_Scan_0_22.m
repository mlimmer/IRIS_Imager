...
% rounded
% 0.20 - modified from app version to create standalone .m file
% 0.21 - Added NTCHS calculations
% 0.22 - Split readtable into numeric and text

%% Constants - read from file
fprintf('Loading parameters...\n');

ParamFile='AdjustableParameters-test.xlsx'; %Parameter file name
Parameters_Num=readtable(ParamFile,'Sheet','Parameters','Range','A1:B21','ReadVariableNames',0,'ReadRowNames',1);
Parameters_Text=readtable(ParamFile,'Sheet','Parameters','Range','A22:B27','ReadVariableNames',0,'ReadRowNames',1);

DPI=(Parameters_Num.Var1(1)); %image resolution in pixels/inch
x_Crop=(Parameters_Num.Var1(2)); %number of pixels to crop off x direction
y_Crop=(Parameters_Num.Var1(3)); %number of pixels to crop off y direction
Scale_Factor=(Parameters_Num.Var1(4)); %Ratio to scale the image by averaging pixels i.e., 0.5 = 1/2 size
Start_Upside_down_flag=(Parameters_Num.Var1(5)); %Flag for upsidedown scanned in images 1=upsidedown
End_Upside_down_flag=(Parameters_Num.Var1(6)); %Flag for upsidedown scanned in images 1=upsidedown
White_L=(Parameters_Num.Var1(7)); %Lightness value for white film (LAB)
Fe_b_Threshold=(Parameters_Num.Var1(8)); %Values of b* change above this are considered Fe (rather than white) for counting Fe on Mn films
CircleRad_min=(Parameters_Num.Var1(9))*DPI*Scale_Factor; %Min cirlce radius in pixels;
CircleRad_max=(Parameters_Num.Var1(10))*DPI*Scale_Factor; %Max cirlce radius in pixels;
Crop_Buffer=(Parameters_Num.Var1(11)); %Factor for cropping circles to increase image area by a factor of 2
Circle_NaN_buffer=(Parameters_Num.Var1(12)); %Factor for increasing NaN masking around circles (multiplied by circle radius)
n_circles_start=(Parameters_Num.Var1(13)); %number of circles in starting image
n_circles_end=(Parameters_Num.Var1(14)); %number of circles in ending image
n_clusters_start=(Parameters_Num.Var1(15)); %number of clusters to find in starting image
n_clusters_end=(Parameters_Num.Var1(16)); %number of clusters to find in ending image
Avg_Thickness=(Parameters_Num.Var1(17)); %Technical Standard for Hydric Soils: Thickness of zone to average removal across (in)
Avg_Depth=(Parameters_Num.Var1(18)); %Technical Standard for Hydric Soils: Bottom of zone to average removal across (in)
OutputRemoval=(Parameters_Num.Var1(19)); %flag: 1 to output removal with depth, 0 to supress output
OutputRemovalDepth=(Parameters_Num.Var1(20)); %Distance over which to average removal with depth (inches). Use 0 without averaging
IRIS_Type=Parameters_Text.Var1{1}; %Type of IRIS film analyzed -- Fe, Mn or S
OutFile=Parameters_Text.Var1{2}; %Output data file name
OutSheet=Parameters_Text.Var1{3}; %Output datasheet
Start_circle_color=Parameters_Text.Var1{4}; %Color of circle relative to sheet for starting image
End_circle_color=Parameters_Text.Var1{5}; %Color of circle relative to sheet for ending image
Image_Path=Parameters_Text.Var1{6}; %Default file path for images
...
if ~strcmp(IRIS_Type,'S') %Start file not needed for S films
    ...
    if ~ischar(Start_file)
        (fprintf('No start file selected\n'));
...
if ~ischar(End_file)
    (fprintf('No end file selected\n'));
    
    return;
end

(fprintf('Files selected, checking to see if processed files exist.\n'));
...
if exist(filename,'file')==2
    (fprintf('Output file already exists!  Loading output file.\n'));
    ...
    switch IRIS_Type
        case 'S'
            ...
            (fprintf('Saving image files as .mat\n'));
        ...
        otherwise
            (fprintf('Improper IRIS film type specified in "AdjustableParameters" spreadsheet'));
...
switch IRIS_Type
    case {'Mn','Fe'}
        ...
        I_start_Munsell=MUNSELL_MATCH(I_start_clustercolors, ParamFile);
        ...
        I_end_Munsell=MUNSELL_MATCH(I_end_clustercolors, ParamFile);
...
%% Output Data
(fprintf('Writing output data to Excel...\n'));
...
if isfile(OutFile)
    ...
    existing_rows=1;
    (fprintf('Output file does not exist. Writing headers...\n'));
    ...
    switch IRIS_Type %Adjust headers for each element
        case 'Mn'
            Headers={'Start File Name','End File Name','Sheet Width (in)','Sheet Length (in)','Mean % Removal','Median % Removal'...
                ...
                'Percent of Film with Fe pixels (for Mn films)', 'Mean % Removal assuming Fe pixels as 100% removal (for Mn Films)','Median % Removal assuming Fe pixels as 100% removal (for Mn Films)','HSTS Max Removal assuming Fe pixels as 100% removal',...
        ...
        case 'Fe'
             Headers={'Start File Name','End File Name','Sheet Width (in)','Sheet Length (in)','Mean % Removal','Median % Removal','HSTS Max Removal',...
            'Mean L value of ending film','Mean vertical standard deviation of removal','Mean horizontal standard deviation of removal',...
            'Overall standard deviation of removal','Removal centroid in x','Removal centroid in y',...          
            'Film change in a*', 'Film change in b*', 'Starting film cluster areas (%)','','','Starting film Munsell cluster colors','','',...
            'Ending film cluster areas (%)','','','','','Ending film Munsell cluster colors','','','','',...
...
switch IRIS_Type %Adjust output data based upon film type
    case 'Mn'
        ...
        %Calculate Technical Standards for Hydric Soils
        Pixel_per_inch=length(y_length)/y_length(length(y_length)); %ratio of pixel per inch
        Avg_Depth=min(y_length(length(y_length)),Avg_Depth);
        Avg_Thickness=min(Avg_Thickness,Avg_Depth);
        HSTS_Thick=floor(Avg_Thickness*Pixel_per_inch);%Thickness of window
        HSTS_Depth=floor(Avg_Depth*Pixel_per_inch); %bottom of window to average to
        HSTS_window_start=1:floor(HSTS_Depth-HSTS_Thick);
        if isempty(HSTS_window_start)
            HSTS_max_noFe={'Film is smaller than window'};
        else
            HSTS_window=[HSTS_window_start',HSTS_window_start'+HSTS_Thick];
            HSTS_Averages=mean(mean_I_L_diff_y_noFe(HSTS_window),2);
            HSTS_max_noFe=max(HSTS_Averages);
        end
        ...
        Outdata=table({Start_file}, {End_file},x_width(length(x_width)), y_length(length(y_length)),...
            ...
            Fe_Percent, mean_I_L_diff_noFe, median_I_L_diff_noFe,HSTS_max_noFe,...
    ...
    case 'Fe'
        ...
        HSTS_window_start=1:floor(HSTS_Depth-HSTS_Thick);
        if isempty(HSTS_window_start)
            HSTS_max={'Film is smaller than window'};
        else
            HSTS_window=[HSTS_window_start',HSTS_window_start'+HSTS_Thick];
            HSTS_Averages=mean(mean_I_L_diff_y(HSTS_window),2);
            HSTS_max=max(HSTS_Averages);
        end
        ...
        % cluster area, ending cluster colors, Removal fit values
        Outdata=table({Start_file}, {End_file}, x_width(length(x_width)), y_length(length(y_length)),...
            mean_I_L_diff, median_I_L_diff,HSTS_max, ...
            mean_I_L_val, mean_std_I_L_diff_x, mean_std_I_L_diff_y,...
            mean_std_I_L_diff, x_cent_I_L_diff, y_cent_I_L_diff,...
            mean_I_a_diff, mean_I_b_diff,...
            Start_cluster_area, I_start_Munsell', num2cell(End_cluster_area), I_end_Munsell',...
...
if OutputRemoval==1
   ...
   if OutputRemovalDepth==0
    ...
    elseif OutputRemovalDepth>0
        Pixel_per_inch=length(y_length)/y_length(length(y_length)); %ratio of inches per pixel
        Inc=floor(OutputRemovalDepth*Pixel_per_inch);%increment
        ...
        for i=1:num_depths %perform averaging
            start_index=floor((i-1)*OutputRemovalDepth*Pixel_per_inch)+1;
            end_index=floor(i*OutputRemovalDepth*Pixel_per_inch);
...
function [Color_Match] = MUNSELL_MATCH(C_in, ParamFile)
%MUNSELL_MATCH Matches image color to Munsell soil colors
%   C_in is a matrix of [L*,A*,B*] to be matched in L*A*B* color space
%   where the number of rows is the number of colors to be matched and
%   infile is the input parameters file
